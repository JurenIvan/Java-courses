...
public class GlasanjeChartServlet extends HttpServlet {
	...
	/**
	 * Method that creates data set from data found in database
	 * 
	...
	private DefaultPieDataset getDataset(HttpServletRequest req) throws IOException {
		...
		List<PollOptionModel> records=DAOProvider.getDao().getResultsData(pollID);

		for (var e : records) {
			dataset.setValue(e.getOptionTitle(), e.getVoteCount());
		}
		return dataset;
	}
